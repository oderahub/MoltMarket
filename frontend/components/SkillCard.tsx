...
export default function SkillCard({ title, desc, price, isSbtc, isUsdcx, onExecute }: SkillCardProps) {
  return (
    <div className="group bg-terminal border border-terminal-border p-4 rounded-none hover:bg-white/[0.02] hover:border-stacks transition-all cursor-pointer relative border-l-2 border-l-transparent hover:border-l-stacks">
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex gap-1">
          {isSbtc && (
            <span className="text-[8px] font-bold px-1 border text-stacks border-stacks/30 bg-stacks/10">
              sBTC
            </span>
          )}
          {isUsdcx && (
            <span className="text-[8px] font-bold px-1 border text-blue-400 border-blue-400/30 bg-blue-400/10">
              USDCx
            </span>
          )}
          {!isSbtc && !isUsdcx && (
            <span className="text-[8px] font-bold px-1 border text-white/50 border-white/20 bg-white/5">
              STX
            </span>
          )}
        </div>
